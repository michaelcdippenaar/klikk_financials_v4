...
# one mutates financial data (trial-balance rebuild). Consumed ONLY by the frontend,
# which sends a Bearer JWT (klikk_portal src/api/client.js). The MCP does NOT call
# /xero/cube/*. Do NOT revert to AllowAny. NOTE: xero_data/* stays AllowAny because the
# MCP calls it without KLIKK_API_TOKEN — lock that only after the MCP has a token.
