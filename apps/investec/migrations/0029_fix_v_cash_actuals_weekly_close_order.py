# Fix v_cash_actuals_weekly: rank the week-end CLOSING balance by posting_date,
# not transaction_date.
#
# DEFECT (same class as the Report 1 opening-balance bug, found 2026-06-25):
# The `ranked` CTE picked each account's week-end running_balance by ordering on
#   txn_date DESC, posted_order DESC, id DESC
# where txn_date = COALESCE(transaction_date, posting_date). transaction_date is
# the bank VALUE date (when a txn economically occurred); it can lag, lead, or be
# NULL relative to when the txn POSTS. running_balance is the bank's *cleared*
# balance, which advances with posting_date + posted_order — so ranking on
# transaction_date can select a stale row whose running_balance is a transient
# mid-period figure, not the true week-end cleared balance. (On the live data
# this mis-picks the week-end close on the two MC personal sub-accounts and the
# mortgage loan accounts — the same accounts the Python opening picked wrong.)
#
# FIX: rank by posting_date DESC (with transaction_date as the COALESCE fallback
# only so a value-only row still sorts), then posted_order DESC, then id DESC —
# matching apps/investec/cashflow_forecast._liquid_opening_balance and the
# model's own Meta.ordering = ['-posting_date', '-posted_order'].
#
# Nothing else in the view changes: the column list, order, and every other CTE
# are byte-identical to 0028. CREATE OR REPLACE VIEW is therefore a metadata-only
# operation — no rebuild, no lock on the ~20k-row transaction table.
...
# Reverse: restore the 0028 definition (ranked by txn_date, no posting_date in txn).
REVERSE_SQL = r"""
CREATE OR REPLACE VIEW v_cash_actuals_weekly AS
WITH attr AS (
    SELECT * FROM (VALUES
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("investec", "0028_fix_v_cash_actuals_weekly_instal_token"),
    ]
